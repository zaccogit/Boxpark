...
export default function AuthTokenInternal({ active, setActive, onSubmit }: Props) {
  ...
  return (
    <>
      <Modal
        active={active}
        disabledButton={!channelTypeId}
        onClose={() => {
          setActive(false);
        }}
        onSubmit={() => { setIsActive(true) }}
      >
        <Text style={[styles.textSubTitle, { marginVertical: 30, textAlign: 'center' }]}>
      ...
      <Modal
        ...
        <Text style={[styles.text]}>
          {Languages[language].SCREENS.PreQRPaymentScreen.text10} {channelTypeId === 1 ? 'sms' : 'email'}
        </Text>
        <Input
          placeholder={Languages[language].SCREENS.PreQRPaymentScreen.placeholder1}
          maxLength={6}
          value={tokenAuth}
        ...
        <TouchableOpacity
          ...
          <Text style={[styles.text, { color: timer2 > 0 ? Colors.danger : Colors.green }]}>
            {Languages[language].GENERAL.BUTTONS.textResend} {channelTypeId === 1 ? 'sms' : 'email'}{' '}
