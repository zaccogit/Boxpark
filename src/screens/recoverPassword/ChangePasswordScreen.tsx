...
const ChangePasswordScreen = ({ navigation, route: { params } }: Props) => {
  ...
  const onSubmit = async () => {
    try {
      ...
      let req: any = {
        email:
          params?.reReq?.channelTypeId === 2 ? params?.sesion?.email : null,
        phoneNumber:
          params?.reReq?.channelTypeId === 1 ? params?.sesion?.phone : null,
      ...
      if (params?.sqFail) {
        url = endPoints?.find(
          (endPoint: EndPointsInterface) =>
            endPoint.name === "RECOVER_PASSWORD_SIMPLE_URL"
        )?.vale as string;
        method = endPoints?.find(
          (endPoint: EndPointsInterface) =>
            endPoint.name === "RECOVER_PASSWORD_SIMPLE_METHOD"
        )?.vale as Method;
        req.idetificationNumber = `${params?.sesion?.documentId}`;
      } else {
        url = endPoints?.find(
          (endPoint: EndPointsInterface) =>
            endPoint.name === "RECOVER_PASSWORD_URL"
        )?.vale as string;
        method = endPoints?.find(
          (endPoint: EndPointsInterface) =>
      ...
      if (response?.codigoRespuesta === "00") {
        ...
        ToastCall(
          "warning",
          Languages[language].SCREENS.ChangePasswordScreen.ERRORS.message1,
          language
        );
      } else if (response?.codigoRespuesta === "97") {
        ToastCall(
          "warning",
          Languages[language].SCREENS.ChangePasswordScreen.ERRORS.message2,
          language
        );
      } else {
  ...
  const validatePassword = () => {
    if (credentials.password !== credentials.confirmPassword) {
      ToastCall(
        "warning",
        Languages[language].SCREENS.ChangePasswordScreen.ERRORS.message4,
        language
      );
      return;
    }
  ...
  useEffect(() => {
    if (!params?.sesion || !params?.tokenUser || !params?.reReq || !tokenRU) {
      ...
      <View style={styles.containerForm}>
        <View style={{ width: "100%" }}>
          ...
          <Text style={[styles.subTitle]}>
            ...
            <Text
              style={[styles.subTitle, { color: Colors.green, fontSize: 14 }]}
            >
              {" "}
              # ? ! $ % & * - . ,
            </Text>
        ...
        <View style={{ marginTop: 10 }}>
          <Input
            placeholder={
              Languages[language].SCREENS.ChangePasswordScreen.placeholder1
            }
          ...
          <Input
            placeholder={
              Languages[language].SCREENS.ChangePasswordScreen.placeholder2
            }
            secureTextEntry={true}
            placeholderColor={Colors.gray}
            maxLength={12}
